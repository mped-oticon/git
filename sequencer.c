...
static int do_label(struct repository *r, const char *name, int len)
{
	...
	int ret = 0;
	struct object_id head_oid;
	int len_trunc = utf8_bytes_truncate_line(name, GIT_MIN(len, GIT_MAX_LABEL_LENGTH));
...
{
	int len_trunc = utf8_bytes_truncate_line(label, GIT_MIN(len, GIT_MAX_LABEL_LENGTH));
	struct commit *commit;
	struct object_id oid;
